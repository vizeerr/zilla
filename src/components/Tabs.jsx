  ...
  return (
    <div className='bg-[#30313AE0] mt-[1vw] flex md:flex-row md:gap-0  gap-[3vw] flex-col justify-between items-center md:py-[0.6vw] py-[2.1vw] md:px-[1vw] px-[3.3vw]'>
        <div className='flex md:gap-[0.6vw] gap-[2.4vw]'>
          <button onClick={()=>selectTab(0)} className={`${selected==0? 'bg-primary  text-[#3F3F41]': 'bg-[#4C4C5A] text-white'} anim md:px-[1.5vw] px-[3.4vw] font-[400]  rounded-full font-bebasneue  md:text-[1.1rem] text-[3.1vw] md:pt-[0.2vw] md:pb-[0.1vw] pt-[0.5vw]`}>LAUNCHED TIME</button>
          <button onClick={()=>selectTab(1)} className= {`${selected==1?'bg-primary text-[#3F3F41]': 'bg-[#4C4C5A] text-white'}  anim md:px-[1.5vw] px-[3.4vw] font-[400]  rounded-full font-bebasneue  md:text-[1.1rem] text-[3.1vw] md:pt-[0.2vw] md:pb-[0.1vw] pt-[0.5vw]`}>TRADING VOLUME</button>
          <button onClick={()=>selectTab(2)} className={`${selected==2?'bg-primary text-[#3F3F41]': 'bg-[#4C4C5A] text-white'}  anim md:px-[1.5vw] px-[3.4vw] font-[400]  rounded-full font-bebasneue  md:text-[1.1rem] text-[3.1vw] md:pt-[0.2vw] md:pb-[0.1vw] pt-[0.5vw]`}>MARKET CAP</button>
          <button onClick={()=>selectTab(3)} className={`${selected==3?'bg-primary text-[#3F3F41]': 'bg-[#4C4C5A] text-white'}  anim md:px-[1.5vw] px-[3.4vw] font-[400]  rounded-full font-bebasneue  md:text-[1.1rem] text-[3.1vw] md:pt-[0.2vw] md:pb-[0.1vw] pt-[0.5vw]`}>24H PRICE INC.</button>
        </div>
        <div className=" flex items-center md:gap-[1.2vw] gap-[2vw] relative">
            <div className="bg-white flex justify-between md:gap-[0.2vw] md:pt-[0.1vw] md:pb-[0.2vw] pt-[0.6vw] pb-[0.8vw] md:w-[24vw] w-[62vw] rounded-full items-center md:ps-[0.7vw] md:pe-[0.6vw] ps-[1.5vw] pe-[1.5vw] overflow-hidden ">
                ...
                <input type="text" placeholder='SEARCH TOKENS' className="focus:border-0 active:border-0 md:text-[0.8rem] text-[2.1vw] font-[800] font-montserrat w-full text-center border-0 outline-none " />
            </div>
